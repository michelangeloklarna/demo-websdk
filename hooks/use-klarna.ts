...
interface UseKlarnaOptions {
  ...
  locale?: string
  loadOnMount?: boolean
  onLog?: (type: "info" | "success" | "warning" | "error", title: string, message: string, data?: any) => void
}
...
export function useKlarna({ amount, currency = "USD", locale = "en-US", loadOnMount = true, onLog }: UseKlarnaOptions): UseKlarnaReturn {
  ...
  const log = (type: "info" | "success" | "warning" | "error", title: string, message: string, data?: any) => {
    if (onLog) {
  ...
  const loadKlarnaSDK = async () => {
    try {
      ...
      const importKlarna = new Function('return import("https://js.klarna.com/web-sdk/v2/klarna.mjs")')
      ...
      if (!KlarnaSDK || typeof KlarnaSDK !== 'function') {
        throw new Error(`KlarnaSDK not found or invalid. Available exports: ${Object.keys(module || {}).join(', ')}`)
      }
      ...
      log("success", "SDK Initialized", "Klarna SDK instance created successfully", {
        ...
        availableMethods: klarna ? Object.keys(klarna).filter(key => typeof klarna[key] === 'object') : []
      })
      ...
      // Get Payment Presentation element - following Klarna Network Distribution guidelines
      const presentationConfig = {
        amount: Math.round(amount * 100), // Convert to minor units (cents)
        currency: currency.toUpperCase(), // Ensure currency is uppercase
        locale,
        intents: ["PAY"], // Customer intent for the payment
        // Optional: Add purchase context if available
        // purchase_country: "US", // Derived from locale if needed
        // purchase_type: "BUY" // Default purchase type
      }

      log("info", "Payment Presentation", "Requesting payment presentation data with Klarna Network Distribution format", presentationConfig)
      ...
      if (klarna.Payment && typeof klarna.Payment.presentation === 'function') {
        try {
          ...
          if (presentation) {
            ...
            if (presentation.icon) {
              ...
              console.log("🖼️ Has Icon Component:", typeof presentation.icon.component === 'function')
            }
            ...
            if (presentation.header) {
              ...
              console.log("📰 Has Header Component:", typeof presentation.header.component === 'function')
            }
            ...
            if (presentation.subheader) {
              ...
              if (presentation.subheader.short) {
                ...
                console.log("📝 Has Short Component:", typeof presentation.subheader.short.component === 'function')
              }
              
              if (presentation.subheader.enriched) {
                console.log("📝 Enriched Subheader:", presentation.subheader.enriched)
                console.log("📝 Has Enriched Component:", typeof presentation.subheader.enriched.component === 'function')
              }
            }
            ...
            if (presentation.paymentButton) {
              ...
              console.log("🔘 Has Payment Button Component:", typeof presentation.paymentButton.component === 'function')
            }
          ...
          if (isValidPresentation) {
            ...
            log("warning", "Invalid Presentation", "Received presentation data doesn't match expected structure", {
              presentationKeys: Object.keys(presentation || {}),
          ...
          log("error", "Presentation Error", "Failed to get payment presentation", {
            error: presentationError instanceof Error ? presentationError.message : String(presentationError)
          })
        ...
        try {
          ...
          log("success", "Presentation Retrieved", "Payment presentation data received via direct method", {
            instruction: presentation?.instruction,
            hasIcon: !!presentation?.icon,
            hasHeader: !!presentation?.header,
          ...
          log("error", "Direct Presentation Error", "Failed to get payment presentation via direct method", {
            error: presentationError instanceof Error ? presentationError.message : String(presentationError)
          })
          throw presentationError
        }
        ...
        const availableMethods = Object.keys(klarna).map(key => ({
          ...
          hasNestedMethods: typeof klarna[key] === 'object' && klarna[key] ? Object.keys(klarna[key]) : []
        }))
        ...
        log("warning", "Payment Method Not Found", "Expected Payment.presentation method not found", {
          availableMethods,
        ...
        presentation = {
          ...
          icon: {
            component: (options = {}) => ({
              mount: (selector: string) => {
                ...
                return { 
                  unmount: () => log("info", "Mock Icon Unmount", `Icon unmounted from ${selector}`)
                }
              }
            })
          },
          header: {
            component: () => ({
              mount: (selector: string) => {
                log("info", "Mock Header Mount", `Header component would mount to ${selector}`)
                return { 
                  unmount: () => log("info", "Mock Header Unmount", `Header unmounted from ${selector}`)
                }
              }
            })
          },
          subheader: {
            short: {
              component: () => ({
                mount: (selector: string) => {
                  log("info", "Mock Short Subheader Mount", `Short subheader would mount to ${selector}`)
                  return { 
                    unmount: () => log("info", "Mock Short Subheader Unmount", `Short subheader unmounted from ${selector}`)
                  }
                }
              })
            },
            enriched: {
              component: () => ({
                mount: (selector: string) => {
                  log("info", "Mock Enriched Subheader Mount", `Enriched subheader would mount to ${selector}`)
                  return { 
                    unmount: () => log("info", "Mock Enriched Subheader Unmount", `Enriched subheader unmounted from ${selector}`)
                  }
                }
              })
            }
          ...
          paymentButton: {
            component: (config = {}) => ({
              mount: (selector: string) => {
                log("info", "Mock Payment Button Mount", `Payment button would mount to ${selector}`, config)
                return { 
                  unmount: () => log("info", "Mock Payment Button Unmount", `Payment button unmounted from ${selector}`)
                }
              }
            })
          }
  ...
  const retry = () => {
    if (amount > 0) {
      log("info", "Retry Initiated", "Retrying Klarna SDK initialization", { amount, currency, locale })
      loadKlarnaSDK()
    }
  }
